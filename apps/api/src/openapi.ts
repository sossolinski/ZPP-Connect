export const openApiDocument = {
  ...
  paths: {
    ...
    "/matching-records": { get: { summary: "List read-only Release compatibility projections for human-confirmed matches" } },
    "/matching/queue": { get: { summary: "List server-paged current relationship claims by matching state" } },
    "/matching/claims/{claimId}": { get: { summary: "Get matching context, persisted suggestions and human decision history" } },
    "/matching/claims/{claimId}/suggestions": { get: { summary: "List immutable persisted match suggestions" } },
    "/matching/claims/{claimId}/suggestions/generate": { post: { summary: "Generate a narrowed explainable suggestion set without confirming a match" } },
    "/matching/claims/{claimId}/candidates": { get: { summary: "Search incident-scoped Passenger candidates for manual review" } },
    "/matching/claims/{claimId}/confirm": { post: { summary: "Append an idempotent human match confirmation" } },
    "/matching/claims/{claimId}/reject": { post: { summary: "Append an idempotent human suggestion rejection" } },
    "/matching/claims/{claimId}/invalidate": { post: { summary: "Invalidate a current human match decision" } },
    "/releases": { get: { summary: "List read-only legacy compatibility projections for Release consumers" } },
    "/releases/queue": { get: { summary: "List the server-paged ReleaseAction queue with dynamic safety state" } },
    "/releases/candidates": { get: { summary: "List current human matches available for a controlled ReleaseAction" } },
    "/releases/{id}": { get: { summary: "Get Passenger, NOK, match, independent checks and current release preconditions" } },
    "/releases/prepare": { post: { summary: "Prepare an idempotent ReleaseAction without authorizing or completing it" } },
    "/releases/{id}/checks/identity": { post: { summary: "Record an explicit human identity ReleaseCheck" } },
    "/releases/{id}/checks/hold": { post: { summary: "Review current Passenger hold state without mutating it" } },
    "/releases/{id}/authorize": { post: { summary: "Freshly revalidate and authorize an eligible ReleaseAction" } },
    "/releases/{id}/complete": { post: { summary: "Record explicit operational completion after fresh revalidation" } },
    "/releases/{id}/cancel": { post: { summary: "Cancel a prepared or authorized ReleaseAction" } },
